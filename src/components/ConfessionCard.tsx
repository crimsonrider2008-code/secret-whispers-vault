import { Play, Trash2, Clock, FileText, Pin, Pause } from "lucide-react";
...
export const ConfessionCard = ({ confession, onPlay, onDelete, onTogglePin, isPlaying }: ConfessionCardProps) => {
  ...
  return (
    <Card className={`glass-effect p-4 hover:bg-secondary/50 transition-all ${confession.isPinned ? 'ring-1 ring-primary/30' : ''}`}>
      <div className="flex items-start gap-4">
        {isTextConfession ? (
          ...
          <Button
            ...
            size="icon"
            onClick={onPlay}
            className={`shrink-0 w-12 h-12 rounded-full ${isPlaying ? 'bg-primary/40' : 'bg-primary/20'} hover:bg-primary/30`}
          >
            {isPlaying ? (
            ...
            <div className="flex items-center gap-1">
              {onTogglePin && (
                <Button
                  ...
                  size="icon"
                  onClick={onTogglePin}
                  className={`shrink-0 h-8 w-8 ${confession.isPinned ? 'text-primary' : 'text-muted-foreground hover:text-primary'}`}
                >
                  <Pin className={`w-4 h-4 ${confession.isPinned ? 'fill-current' : ''}`} />
                </Button>
              )}
              <Button
                variant="ghost"
